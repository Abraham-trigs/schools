...
export async function PUT(
  ...
  try {
    ...
    if (typeof stepIndex !== "number" || !StepSchemas[stepIndex])
      return NextResponse.json({ error: "Invalid or missing step index" }, { status: 400 });

    if (stepIndex === 0)
      return NextResponse.json({ error: "Step 0 updates are not allowed here" }, { status: 400 });
...
/*
Design reasoning:
- Step 0 creation is POST-only on /api/admissions
- All subsequent steps are PUT /:id
- Validation is schema-driven per step
- Progress is server-derived to prevent client tampering
...
- Client must send { step: number, ...payload }
- StepSchemas enforce allowed fields per step
- Transactions protect partial writes

Scalability insight:
- Step logic is schema-driven; new steps require no route changes
- Can integrate audit logs, workflow transitions, or multi-step workflows cleanly
*/
